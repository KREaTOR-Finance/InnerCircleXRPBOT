/**
 * Script to schedule periodic updates of project names
 * This can be run as a cron job to ensure project names are kept up-to-date
 */
...
function startScheduler() {
    console.log('Starting project name update scheduler');
    
    // Schedule the update to run every day at 3:00 AM
    cron.schedule('0 3 * * *', () => {
        console.log('Running scheduled project name update:', new Date().toISOString());
        updateProjectNames();
    });
    
    // Also run it once immediately
    console.log('Running initial project name update');
    updateProjectNames();
}
